...
st.set_page_config(page_title="Questrade 全视角交易终端", layout="wide")
    ...
    with tab2:
        ...
        if not orders:
            ...
            if df_orders.empty:
                ...
                df_orders.rename(columns={'symbol': '股票代码', 'side': '买/卖', 'totalQuantity': '数量', 'limitPrice': '挂单价格', 'state': '状态', 'updateTime': '更新时间'}, inplace=True)
        ...
        if not df_edited.empty and calc_symbol:
            ...
            if calc_mode == "买入 (Buy)":
                ...
                if avail_cash >= total_value:
                    st.success(f"✅ 资金充足！按照此网格买入后预计剩余现金: **${avail_cash - total_value:,.2f} {currency}**")
                else:
                    st.error(f"❌ 资金不足警告！您还需要充值或卖出其他股票来填补资金缺口: **${total_value - avail_cash:,.2f} {currency}**")
                ...
                if pos:
                    ...
                    if current_qty >= total_shares:
                        ...
                        st.error(f"❌ 持仓不足警告！您只持有 {current_qty} 股，无法满足 {total_shares} 股的抛售计划。")
                else:
                    st.warning(f"⚠️ 账户中未查找到 {calc_symbol} 的持仓记录。此网格将被视为做空交易计划。")
